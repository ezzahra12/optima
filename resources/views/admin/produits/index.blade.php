    ...
    <main class="flex-1 p-10">
                ...
                <form action="{{ route('produits.store') }}" method="POST"
                    ...
                    <div>
                        ...
                        <input type="number" name="quantite" required
                            class="w-full border-gray-300 rounded-lg text-sm focus:ring-blue-500" placeholder="0">
                    </div>
                    <div>
                    ...
                    <button type="submit"
                        class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-lg transition shadow-md">
                        Enregistrer
                    </button>
                </form>
            </div>
                    ...
                    <tbody class="divide-y">
                        @foreach ($produits as $produit)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="p-4">
                                    <span
                                        class="bg-gray-100 px-2 py-1 rounded text-[10px] font-bold text-gray-600 uppercase">{{ $produit->categorie }}</span>
                                </td>
                                <td
                                    class="p-4 font-semibold {{ $produit->quantite < 5 ? 'text-red-600' : 'text-gray-700' }}">
                                    {{ $produit->quantite }}
                                </td>
        ...
        @if (session('success'))
            <div id="alert-success"
                class="mb-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-700 font-bold rounded shadow-sm flex justify-between items-center transition-all">
                <span>✅ {{ session('success') }}</span>
                <button onclick="document.getElementById('alert-success').remove()"
                    class="text-green-900 hover:text-black">✕</button>
            </div>
        @endif
    </main>
@endsection
