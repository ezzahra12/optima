...
<body class="bg-gray-100 flex min-h-screen text-sm">
    ...
    <div class="w-60 bg-white border-r border-gray-200 shrink-0">
        ...
        <nav class="px-4">
            ...
            <a href="{{ route('chef.dashboard') }}" class="block p-3 text-gray-600 hover:bg-gray-50 rounded mb-2">
                Tableau de bord
            </a>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <header class="p-10 pb-5">
            <a href="{{ route('chef.dashboard') }}" class="text-indigo-600 text-[10px] font-bold uppercase tracking-widest hover:underline">
                ← Retour aux projets
            </a>
            <div class="flex justify-between items-end mt-2">
                <div>
                    <h2 class="text-3xl font-bold text-gray-900">{{ $projet->nom }}</h2>
                    <span class="text-[10px] font-bold uppercase bg-indigo-50 text-indigo-600 px-3 py-1 rounded border border-indigo-100 mt-2 inline-block">
                ...
                <div class="bg-white border border-gray-200 rounded">
                    ...
                    <form action="{{ route('chef.taches.store') }}" method="POST" class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="titre" required class="w-full border border-gray-200 p-2.5 rounded focus:border-indigo-500 outline-none" placeholder="Ex: Design UI...">
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 uppercase block mb-1">Date Limite</label>
                            <input type="date" name="date_limite" class="w-full border border-gray-200 p-2.5 rounded focus:border-indigo-500 outline-none">
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 uppercase block mb-1">Assigner à</label>
                            <select name="user_id" required class="w-full border border-gray-200 p-2.5 rounded focus:border-indigo-500 outline-none">
                        ...
                        <button type="submit" class="w-full bg-black text-white py-3 rounded font-bold text-[10px] uppercase hover:bg-indigo-600 transition">
                            Créer la tâche
                        </button>
                    </form>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded">
                    ...
                    <div class="p-6">
                        ...
                        <form action="{{ route('chef.projets.addMembre', $projet->id) }}" method="POST" class="flex gap-2">
                            @csrf
                            <select name="user_id" required class="flex-1 border border-gray-200 p-2 rounded text-[10px] outline-none">
                                <option value="">Ajouter...</option>
                                @foreach($all_users as $u)
                                    <option value="{{ $u->id }}">{{ $u->nom }} {{ $u->prenom }}</option>
                                @endforeach
                            </select>
                            <button type="submit" class="bg-indigo-600 text-white px-3 rounded font-bold text-[10px] uppercase">
                                OK
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="divide-y divide-gray-100">
                        ...
                        <div class="p-4 flex items-center justify-between hover:bg-gray-50 transition group">
                            <div class="flex items-center gap-4">
                                <div class="w-8 h-8 rounded bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-[10px]">
                                    {{ $loop->iteration }}
                                </div>
                                <div>
                                    <p class="text-sm font-bold text-gray-800">{{ $tache->titre }}</p>
                                    <p class="text-[10px] text-gray-400 font-bold uppercase">👤 {{ $tache->user?->nom ?? 'Non assigné' }}</p>
                                </div>
                            </div>

                            <div class="flex items-center gap-4">
                                <span class="text-[9px] font-bold px-2 py-1 rounded uppercase border {{ $tache->statut == 'terminé' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-orange-50 text-orange-500 border-orange-100' }}">
                                    {{ $tache->statut }}
                                </span>
