...
<body class="bg-gray-100 flex min-h-screen">
    ...
    <div class="w-60 bg-white min-h-screen border-r border-gray-200 shrink-0">
        ...
        <div class="px-4">
            ...
            <a href="{{ route('employe.dashboard') }}" class="block p-3 text-gray-600 hover:bg-gray-50 rounded mb-2">
                Tableau de bord
            </a>
            <a href="{{ route('employe.taches.index') }}" class="block p-3 bg-indigo-100 text-indigo-700 rounded mb-2 font-bold">
                Kanban Board
            </a>
        ...
        <div class="flex-1 p-10 pt-0 flex gap-6 overflow-x-auto">
            ...
            <div class="w-80 shrink-0">
                ...
                <div class="space-y-4">
                    @forelse($taches->where('statut', $slug) as $tache)
                        <div class="bg-white p-5 rounded border border-gray-200 shadow-sm">
                            ...
                            <div class="flex gap-2 border-t pt-3">
                                <a href="{{ route('employe.taches.show', $tache->id) }}" class="flex-1 text-center py-2 bg-gray-50 text-gray-600 text-[10px] font-bold rounded border uppercase hover:bg-gray-100 transition">
                                    Détails
                                </a>

                                @if($slug !== 'termine')
                                    <form action="{{ route('employe.taches.updateStatus', $tache->id) }}" method="POST" class="flex-1">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="statut" value="{{ $slug == 'a_faire' ? 'en_cours' : 'termine' }}">
                                        <button type="submit" class="w-full bg-black text-white py-2 rounded text-[10px] font-bold uppercase hover:bg-indigo-600 transition">
                    ...
                    @empty
                        <div class="py-10 border-2 border-dashed border-gray-200 rounded text-center text-gray-400 text-xs">
                            Aucune tâche
                        </div>
                    @endforelse
                </div>
